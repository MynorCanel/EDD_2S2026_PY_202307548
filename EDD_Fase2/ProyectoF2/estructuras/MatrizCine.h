...
public:
    std::string fila;
    std::string columna;
    std::string valor; // Puede ser el nombre del cliente que reservó el asiento o un valor vacío si está libre
    Nodo* arriba;
    Nodo* abajo;
    Nodo* izquierda;
    Nodo* derecha;
...
class MatrizCine {
private:
    // Código único de la función para identificarla
    Nodo* cabeza;
    std::string pelicula;
    std::string horario;
    std::string sala;
...
public:
    ...
    MatrizCine(const std::string& codigo);
    ~MatrizCine();
